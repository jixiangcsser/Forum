...
public class UserService {
    ...
    public User getProfile(int sessionUid, int uid) {  //uid是用户id.
        //如果是浏览别人的主页，则增加主页浏览数
        if(sessionUid!=uid){
        ...
        user.setFollowCount((int)(long)jedis.scard(uid+":follow"));//关注的id号的长度
        user.setFollowerCount((int)(long)jedis.scard(uid+":fans"));//粉丝数的个数
        String likeCount = jedis.hget("vote",uid+"");//从hashmap中获取的赞的个数
        if(likeCount==null){
            user.setLikeCount(0);
        }else {
            user.setLikeCount(Integer.valueOf(likeCount)); //设置用户的赞的个数
        }

        if(jedis!=null){
            jedisPool.returnResource(jedis);//在把这个jedis放入池中
        }
        return user;
    }
